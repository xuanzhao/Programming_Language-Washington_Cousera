...
(define (racketlist->mupllist rkxs)
  (if (null? rkxs)
      (aunit)
      (apair (car rkxs) (racketlist->mupllist (cdr rkxs)))))

(define (mupllist->racketlist muxs)
  (if (aunit? muxs)
      null
      (cons (apair-e1 muxs) (mupllist->racketlist (apair-e2 muxs))))) 
         ...
         (let ([clo (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? clo)
               (let* ([clo-fun (closure-fun clo)]
                      [clo-env (cons (cons (fun-formal clo-fun) arg)
                                     (closure-env clo))]
                      [clo-env (if (fun-nameopt clo-fun)
                                   (cons (cons (fun-nameopt clo-fun) clo)
                                         clo-env)
                                   clo-env)])
                 (eval-under-env (fun-body clo-fun) clo-env))
